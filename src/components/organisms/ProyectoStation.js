/**
 * ORGANISM: ProyectoStation
 * Estación que aparece en la escena de proyecto con los módulos: Momentos, Necesidades, Entrevistas, Storyboard
...
import { PROYECTO_MODULE_DEFINITIONS, PROYECTO_HUB_CONFIG, PROYECTO_STATION_CONFIG } from '../../config/proyectoModulesConfig.js';

export class ProyectoStation {
    constructor() {
        ...
        this.moduleDefinitions = PROYECTO_MODULE_DEFINITIONS;
        this.config = PROYECTO_STATION_CONFIG;
    }

    build() {
        // Hub central
        const hubPos = new THREE.Vector3(
            PROYECTO_HUB_CONFIG.position.x,
            PROYECTO_HUB_CONFIG.position.y,
            PROYECTO_HUB_CONFIG.position.z
        );
        this.hub = new StationHub(PROYECTO_HUB_CONFIG.name, hubPos, PROYECTO_HUB_CONFIG.color);
        ...
        for (let i = 0; i < this.moduleDefinitions.length; i++) {
            ...
            // Strut desde el hub al módulo (completamente debajo del piso)
        ...
        const exitStrut = createStrut(
            new THREE.Vector3(0, -2, 0),
            new THREE.Vector3(exitPos.x, -2, exitPos.z),
            0xffaa00
        );
        ...
        const exitCatwalk = new Catwalk(exitFrom, exitPortalPos, 0xffaa00);
        this.catwalks.push(exitCatwalk);
        this.group.add(exitCatwalk.getGroup());

        const exitBeyond = exitPortalPos.clone().add(exitFacing.clone().multiplyScalar(2.2));
        const exitExtCatwalk = new Catwalk(exitPortalPos, exitBeyond, 0xffaa00, 0.08);
        this.catwalks.push(exitExtCatwalk);
        this.group.add(exitExtCatwalk.getGroup());

        // Plataformas de salto y aterrizaje para el portal de salida
        this.createJumpingPads(exitFrom, exitPortalPos, exitAngle);
        this.createLandingPad(exitPortalPos, exitFacing);

        console.log('✓ ProyectoStation built successfully');
    }
    ...
    async loadPortalModels() {
        console.log('ProyectoStation: Loading portal models...');
        ...
        console.log('✓ ProyectoStation: All portal models loaded');
    }
}
